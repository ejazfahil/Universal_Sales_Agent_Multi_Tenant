...
def test_eu_regions_accepted_in_production(region: str) -> None:
    settings = Settings(app_env="production", deployment_region=region, auth_secret="real-secret")
...
def test_non_eu_region_rejected_in_production(region: str) -> None:
    ...
    with pytest.raises(ValueError, match="outside the EEA"):
        Settings(app_env="production", deployment_region=region, auth_secret="real-secret")
...
def test_placeholder_auth_secret_refuses_to_deploy() -> None:
    """A deploy that forgot to set a secret must not sign forgeable tokens."""
    with pytest.raises(ValueError, match="development placeholder"):
        Settings(app_env="production", deployment_region="eu-central-1")


def test_placeholder_auth_secret_is_fine_locally() -> None:
    assert Settings(app_env="local").auth_secret
